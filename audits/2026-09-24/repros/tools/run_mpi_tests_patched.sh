...
run_test() {
    local name="$1"
    local procs="${2:-$NP}"
    local binary="${CARGO_TARGET_DIR:-./target}/${BUILD_MODE}/examples/${name}"
...
# shellcheck disable=SC2086
PERSIST_OUTPUT=$("$MPIEXEC" $MPIEXEC_ARGS -n "$NP" "${CARGO_TARGET_DIR:-./target}/${BUILD_MODE}/examples/persistent_bcast" 2>&1) || true
