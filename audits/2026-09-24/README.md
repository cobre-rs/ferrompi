...
| SND-06 | gather/allgather/scatter never validate buffer sizes (9 methods) | critical | repro | 0.5.x | open |
| SND-07 | V-collectives don't validate counts/displs vs size and buffer | critical | repro | 0.5.x | open |
| SND-08 | `*_custom` p2p: unbounded `T`, unchecked extent | critical | repro | 0.5.x (D-7) | open |
| SND-09 | RMA target range / origin count unvalidated (remote OOB write) | critical | repro | 0.5.x (D-11) | open |
| SND-10 | Window memory used after finalize | critical | repro | 0.5.x (D-8) | open |
| SND-11 | `Communicator` Send+Sync regardless of thread level | critical | repro | 0.5.x (D-3, D-9) | open |
| SND-12 | Uninitialised window memory exposed as `&[T]` | major | repro | 0.5.x | open |
| SND-13 | `SharedWindow` slices over concurrently-written memory (observed miscompile) | critical | repro | 0.6 (D-5); 0.5.x doc warning | open |
| SND-14 | `UserOp` fat-pointer transmute relies on unspecified layout | minor | reading | 0.5.x† | open |

### Correctness — [02](findings/02-correctness.md)

| ID | Title | Sev | Verified | Target | Status |
|---|---|---|---|---|---|
| COR-01 | Error classes decoded with Open MPI numbering (wrong on MPICH) | major | repro | 0.5.x | open |
| COR-02 | Stale request handles act on unrelated requests (ABA) | major | repro | 0.5.x | open |
| COR-03 | No request write-back on error in wait/test-many | major | repro | 0.5.x | open |
| COR-04 | `MPI_STATUSES_IGNORE` loses per-request error | minor | repro | 0.5.x | open |
| COR-05 | Counts > `INT_MAX` silently truncated | critical | repro | 0.5.x | open |
| COR-06 | `MPI_ERRORS_RETURN` not on `MPI_COMM_SELF` | major | reading | 0.5.x | open |
| COR-07 | Finalize/re-init lifecycle aborts; `UserOp` drop after finalize | major | repro | 0.5.x | open |
| COR-08 | Finalize sweep frees active requests / collectively frees windows | minor | reading | 0.5.x | open |
| COR-09 | `-1` = PROC_NULL (MPICH) vs ANY_SOURCE (Open MPI) | minor | repro | 0.6 | open |
| COR-10 | `cancel()` allowed on collective/RMA requests | minor | repro | 0.5.x | open |
| COR-11 | `LongDoubleInt`/`LongInt` layout wrong on macOS arm64 / Windows | minor | reading | 0.5.x | open |
| COR-12 | Pre-MPI-4 stubs never yield `NotSupported`; 3 contradicting docs | minor | reading | 0.5.x | open |
| COR-13 | `Win::sync` rustdoc wrong; example fails on MPICH | minor | repro | 0.5.x docs / 0.6 API | open |
| COR-14 | `MPI_UNDEFINED` from `MPI_Get_count` leaks; rc ignored | nit | reading | 0.5.x | open |
| COR-15 | `op_set_closure` no bounds check; `op_create_user` no `op_used` check | nit | reading | 0.5.x | open |
| COR-16 | `type_create_struct` maybe-uninitialised arrays at count 0 | nit | compiler | 0.5.x | open |
...
| ARC-02 | Non-additive `rma` feature; no `#[non_exhaustive]` anywhere | major | one-way | 0.6 | open |
| ARC-03 | Shim representation in public API (`raw_handle`, discriminant contract, pub `from_code`) | major | one-way | 0.6 | open |
| ARC-04 | Handles not tied to `Mpi` lifetime | major | mixed | 0.5.x guards / 0.6 | open |
| ARC-05 | Thread-safety model inconsistent | major | mixed | 0.5.x (D-3) | open |
...
| ARC-10 | Copy-pasted families drifted (root of SND-06) | major | two-way | 0.5.x (validators) | open |
...
| ID | Title | Sev | Verified | Target | Status |
|---|---|---|---|---|---|
| ABI-01 | build.rs drops `-D` flags (`MPI_ABI` lost → silent ABI mismatch) | critical (latent) | reading | 0.5.x | open |
| ABI-02 | No ABI detection probe; MPICH 4.3 draft ABI accepted | major | reading | 0.5.x reject / 0.7 build | open |
| ABI-03 | Build-selection env vars not tracked (= INF-03) | major | repro | 0.5.x | open |
| ABI-04 | Public-API one-way doors (= ARC-02/03) | major | reading | 0.6 | open |
| ABI-05 | ADR-0001 misstates what the ABI adds; ADR-0006 needed | minor | reading | 0.5.x | open |
| ABI-06 | No interop API (deliberately deferred) | info | — | after ABI backend | open |
| ABI-07 | Optional CI compile+link job against Forum ABI stubs | info | built locally | 0.5.x or 0.7 | open |
...
| PRF-02 | Bitmap concentrates contention; ADR/comment claim the opposite | minor | measured | 0.5.x docs / 0.7 | open |
| PRF-03 | `ffi_overhead` bench cannot measure FFI overhead | minor | measured | 0.5.x | open |
| PRF-04 | "Persistent 10–30% faster" refuted as stated; bench at 1 MiB only | minor | measured | 0.5.x | open |
...
| PRF-06 | Dead per-callback tag lookup; `Vec` per `wait_some`; topology 256·P | nit | reading | 0.5.x | open |
| PRF-07 | `[profile.release]` doesn't reach downstream; comment says it does | minor | Cargo semantics | 0.5.x | open |

### Bloat / overengineering — [06](findings/06-bloat-overengineering.md)

| ID | Title | ~Lines | Target | Status |
|---|---|---:|---|---|
| BLT-01 | 76/181 unit tests catch no regression; 16 compile witnesses | 1,000 | 0.5.x† | open |
| BLT-02 | Private C header restates signatures in Doxygen | 800 | 0.5.x† | open |
| BLT-03 | `MPI_VERSION < 3` branches unreachable | 455 | 0.5.x† | open |
| BLT-04 | 12 near-identical in-place example binaries | 450 | 0.5.x† | open |
| BLT-06 | Example scaffolding duplicated in 26 files | 390 | 0.5.x† | open |
| BLT-08 | Boilerplate SAFETY / marshalling; 59 `unsafe` blocks without SAFETY | 300 | 0.5.x† | open |
| BLT-09 | Three drifted test runners + deprecated `test.sh` | 200 | 0.5.x† | open |
| BLT-10 | 42 redundant `[[example]]` entries | 185 | 0.5.x† | open |
| BLT-11 | Tables/indexes repeated across 3–5 docs; marketing tone | 250 | 0.5.x† | open |
| BLT-14 | 15 in-place C shims differ only by `MPI_IN_PLACE`; dead `is_root` | 300 | 0.5.x† | open |
| BLT-15 | `UserOp` double registry + dead per-callback lookup | 120 | 0.5.x† | open |
| BLT-16 | Benches measuring nothing; duplicated bench protocol | 210 | 0.5.x† | open |
| BLT-20 | Six copies of the slot-claim loop | 70 | superseded by ARC-01 | open |
| BLT-21 | Process artifacts, stale line refs, expired promises in comments | 45 | 0.5.x† | open |
| BLT-23 | `Group::undefined()` FFI call returning literal −1 | 35 | 0.5.x† | open |
| BLT-25 | Dead C branches (errhandler install) | 30 | 0.5.x† | open |
| BLT-26 | `const _` Send/Sync asserts beside `unsafe impl` | 27 | 0.5.x† | open |
| BLT-27 | Dead `ferrompi_init`; module-wide `allow(dead_code)` | 25 | 0.5.x† | open |
| BLT-29 | `with_handles` implemented twice | 20 | 0.5.x† | open |
| BLT-32 | `ReduceOp` compile_fail doctest via 14 `cfg_attr` | 16 | 0.6 (with ARC-02) | open |
| BLT-34 | `use super::*` in 11 test modules | — | 0.5.x† | open |

### Documentation — [07](findings/07-docs.md)

| ID | Title | Sev | Target | Status |
|---|---|---|---|---|
| DOC-01 | `docs/architecture.md` false statements | major | 0.5.x | open |
| DOC-02 | ADR-0001 driver 1 false (= ABI-05) | minor | 0.5.x | open |
| DOC-03 | ADR-0002 claims that do not hold | minor | 0.5.x | open |
| DOC-04 | ADR-0004 lifetime rejection mis-argued; nonexistent variant | minor | 0.5.x note / 0.6 new ADR | open |
| DOC-05 | ADR-0005 Decision 7 describes the rejected design; plan sections | major | 0.5.x | open |
| DOC-06 | Migration guide: nonexistent APIs, false safety claim | major | 0.5.x | open |
| DOC-07 | `docs/mpi-compatibility.md` inaccuracies | minor | 0.5.x | open |
| DOC-08 | `README.md` inaccuracies (badge, version, reqs, `LD_LIBRARY_PATH`) | minor | 0.5.x | open |
| DOC-09 | Crate rustdoc (`src/lib.rs`) inaccuracies | minor | 0.5.x | open |
| DOC-10 | Stale/wrong item-level rustdoc | minor | 0.5.x | open |
| DOC-11 | CONTRIBUTING template filler + false policies | minor | 0.5.x | open |
| DOC-12 | CHANGELOG internal IDs / restated rustdoc | minor | 0.5.x | open |
| DOC-13 | `benches/README.md` claims | minor | 0.5.x | open |
| DOC-14 | Missing docs: lifecycle, error reporting, runtime lib path, Open MPI build | minor | 0.5.x | open |
| DOC-15 | Wrong C comments | nit | 0.5.x | open |

### Build / CI / tests — [08](findings/08-build-ci-tests.md)

| ID | Title | Sev | Verified | Target | Status |
|---|---|---|---|---|---|
| INF-01 | MPI test runner passes when it ran nothing | major | repro | 0.5.x | open |
| INF-02 | Declared MSRV false; no MSRV job | major | repro | 0.5.x (D-12: 1.85) | open |
| INF-03 | build.rs never re-runs on MPI selection change | major | repro | 0.5.x | open |
| INF-04 | build.rs precedence contradicts docs; overrides fail silently | major | repro | 0.5.x | open |
| INF-05 | build.rs dead/misleading output | minor | reading | 0.5.x | open |
| INF-06 | No `links` manifest key | minor | reasoning | 0.5.x | open |
| INF-07 | docs.rs builds default features only (no RMA docs) | major | live check | 0.5.x | open |
| INF-08 | Doctests never run on PRs | major | reading | 0.5.x | open |
| INF-09 | MPI-4 probes turn regressions into silent passes | major | reading | 0.5.x | open |
| INF-10 | Odd-np deadlocks; CI np=4 only | minor | repro | 0.5.x | open |
| INF-11 | No large-count integration test | major | reading | 0.5.x | open |
| INF-12 | Public APIs untested; examples never run | minor | reading | 0.5.x | open |
| INF-13 | Error-class assertions accept any class | major | reading | 0.5.x | open |
| INF-14 | Release notes extraction yields empty bodies | minor | repro | 0.5.x | open |
| INF-15 | CI coverage gaps (numa clippy, sanitizers, coverage 11/73) | minor | reading | 0.5.x | open |
| INF-16 | MPICH hotfix: unchecked downloads, copy-pasted ×4 | minor | reading | 0.5.x | open |
| INF-17 | `security.yml` hygiene | nit | reading | 0.5.x | open |
| INF-18 | Package ships repo internals (incl. `audits/`) | minor | `cargo package` | 0.5.x | open |
| INF-19 | ADR-0002 mandated TSan step missing | minor | reading | 0.5.x / moot after ARC-01 | open |
| INF-20 | CI covers MPICH 4.2 + Open MPI 4.x only (no Open MPI 5) | minor | reading | 0.5.x | open |
